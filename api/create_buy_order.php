...
if(!$buyer_id || !$item_name || $price<=0){http_response_code(400);echo json_encode(['success'=>false,'message'=>'buyer_id, item_name ve price zorunlu']);exit;}
...
// Alıcının bakiyesini kontrol et (yeterli olmalı ama para çekimi fulfillment sırasında yapılacak)
$stmt=$pdo->prepare('SELECT id,balance FROM user WHERE id=?');$stmt->execute([$buyer_id]);$u=$stmt->fetch();if(!$u){http_response_code(404);echo json_encode(['success'=>false,'message'=>'Kullanıcı bulunamadı']);exit;}
$ins=$pdo->prepare('INSERT INTO buy_orders (buyer_id,item_name,price,status) VALUES (?,?,?,?)');$ins->execute([$buyer_id,$item_name,$price,'active']);
echo json_encode(['success'=>true,'message'=>'Sipariş oluşturuldu','order_id'=>$pdo->lastInsertId()]);
